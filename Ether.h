...
//
//#define UDP_PROTOCOL 17    // IPv4协议号：UDP=17
//
//
//struct ADDRESS {
//    uint32_t ipv4;
...
//};
//
//// 补充UDP头部结构体（紧跟在IcmpHeader后）
//struct UdpHeader {
//    uint16_t src_port;      // 源端口
//    uint16_t dst_port;      // 目的端口
//    uint16_t len;           // UDP总长度（头部+数据）
//    uint16_t checksum;      // UDP校验和（可选，填0则不校验）
//};
//
//// 补充UDP相关宏定义（紧跟在ICMP_HDR_LEN后）
//
//
...
//    }
//
//    // 计算UDP校验和（包含伪头部，RFC 768标准）
//    uint16_t calculate_udp_checksum(const uint32_t src_ip, const uint32_t dst_ip, const uint8_t* udp_data, int udp_len) const {
//        // 1. 构造UDP伪头部
...
//        pseudo_hdr.zero = 0;
//        pseudo_hdr.protocol = UDP_PROTOCOL;
//        pseudo_hdr.udp_len = htons(udp_len);
//
//        // 2. 拼接伪头部+UDP数据计算校验和
//        uint32_t sum = 0;
//        // 先算伪头部
//        uint16_t* pseudo_ptr = (uint16_t*)&pseudo_hdr;
//        for (int i = 0; i < sizeof(UdpPseudoHeader) / 2; i++) {
//            sum += ntohs(pseudo_ptr[i]);
//        }
//        // 再算UDP数据
//        uint16_t* udp_ptr = (uint16_t*)udp_data;
//        int udp_words = udp_len / 2;
//        for (int i = 0; i < udp_words; i++) {
//            sum += ntohs(udp_ptr[i]);
//        }
//        // 处理奇数长度
//        if (udp_len % 2 != 0) {
//            sum += ((uint16_t)udp_data[udp_len - 1]) << 8;
//        }
//
//        // 折叠校验和
//        sum = (sum >> 16) + (sum & 0xFFFF);
...
//    }
//
//    // 构建UDP数据包（包含以太网+IPv4+UDP头部）
//    int build_udp_frame(
//        uint8_t* frame,               // 输出缓冲区（至少1514字节）
//        const ADDRESS& src_addr,      // 源IP+MAC
//        const ADDRESS& dst_addr,      // 目的IP+MAC
//        uint16_t src_port,            // 源端口
//        uint16_t dst_port,            // 目的端口
//        const uint8_t* payload,       // UDP负载数据
//        int payload_len,
//        bool enable_checksum = true   // 是否启用UDP校验和
//    ) {
//        // 计算各层长度
//        const int UDP_TOTAL_LEN = UDP_HDR_LEN + payload_len;
//        const int IP_TOTAL_LEN = IP_HDR_LEN + UDP_TOTAL_LEN;
//        const int FRAME_TOTAL_LEN = ETH_HDR_LEN + IP_TOTAL_LEN;
//
//        // 校验最大长度（以太网MTU限制）
//        if (FRAME_TOTAL_LEN > 1514) {
//            fprintf(stderr, "UDP frame too large (max 1514 bytes)\n");
//            return -1;
//        }
//
//        uint8_t udp_header[1514];
//        // 3. 构建UDP头部
//        UdpHeader* udp = (UdpHeader*)udp_header;
...
//        // 5. 计算UDP校验和（如需）
//        if (enable_checksum) {
//            udp->checksum = calculate_udp_checksum(src_addr.ipv4, dst_addr.ipv4, (uint8_t*)udp, UDP_TOTAL_LEN);
//        }
//        return build_ipv4_frame(frame, src_addr, dst_addr, UDP_PROTOCOL, udp_header, UDP_TOTAL_LEN);
//    }
//
...
//                mac.send_icmp_request(entry.audio_addr, ip->dst_ip);
//                return;
//            }
...
//            // 提取UDP头部
//            UdpHeader* udp = (UdpHeader*)payload_ptr;
//            int udp_payload_len = ntohs(udp->len) - UDP_HDR_LEN;
//            uint8_t* udp_payload = payload_ptr + UDP_HDR_LEN;
//
//            // 打印UDP信息（可选）
...
//                print_hex_ascii(udp_payload, udp_payload_len);
//            }
//
//            // 可在此处添加自定义UDP数据处理逻辑
//            // handle_udp_data(udp, udp_payload, udp_payload_len, src_addr, dst_addr);
//        }
//    }
//
...
//    ) {
//        if (!dev_handle_) {
//            fprintf(stderr, "Network device not opened!\n");
//            return false;
//        }
//
//        uint8_t frame[1514] = { 0 }; // 以太网最大帧长
//        int frame_len = build_udp_frame(
//            frame, src_addr, dst_addr,
//            src_port, dst_port,
//            payload, payload_len,
//            enable_checksum
//        );
//
//        if (frame_len <= 0) {
//            fprintf(stderr, "Failed to build UDP frame\n");
//            return false;
//        }
//
//        // 打印数据包信息（可选）
//        print_packet_info(
//            src_addr.ipv4, dst_addr.ipv4,
//            src_addr.mac, dst_addr.mac,
//            payload, payload_len,
//            false, true // true=发送包
//        );
//
//        // 调用pcap发送数据包
//        int ret = pcap_sendpacket(dev_handle_, frame, frame_len);
//        if (ret != 0) {
